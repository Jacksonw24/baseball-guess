        ...
        try:
            ...
            for i in range(0, len(arr), 2):
                try:
                    ...
                    continue
            # Total plays per shown user (best-effort, single batched call)
            rounds_map: dict[str, int] = {}
            try:
                if scores:
                    pairs = []
                    for s in scores: pairs.extend([ROUNDS_KEY, s["username"]])
                    # Issue one ZSCORE per username; could pipeline but stdlib doesn't easily
                    for s in scores:
